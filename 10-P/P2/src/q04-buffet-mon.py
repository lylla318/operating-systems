...
class BuffetRestaurant(MP):
    ...
    def __init__(self, max_food):
        ...
        #self.debug = True
        self.numfood = self.Shared("numfood", 0)
        self.maxfood = self.Shared("maxfood", 4)
        self.foodqueue = self.Shared("foodqueue", [])
        self.buffetLock  = self.Lock("monitor lock")
        self.hungrycustomer   = self.buffetLock.Condition("customer can eat")
        self.busycook = self.buffetLock.Condition("cook and make more food")
